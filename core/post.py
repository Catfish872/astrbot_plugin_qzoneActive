...
class PostManager:
    ...
    async def get_latest_approved_by_uin(self, uin: int) -> typing.Optional[Post]:
        """根据 uin 查询最新一条已发布的稿件"""
        async with aiosqlite.connect(self.db_path) as db:
            query = "SELECT * FROM posts WHERE uin = ? AND status = 'approved' ORDER BY create_time DESC LIMIT 1"
            async with db.execute(query, (uin,)) as cursor:
                row = await cursor.fetchone()
                return self._row_to_post(row) if row else None
